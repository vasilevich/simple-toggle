class BotControl {
    ...
    static getAuthorizationHeader() { this.ensureConfigured(); return {Authorization: `Bearer ${this.token}`}; }
    ...
    static async postRequest(fullUrl, obj) { return this.result(await this.request(fullUrl, {method: 'POST',headers: {'Content-Type': 'application/json'},body: JSON.stringify(obj ?? {})})); }
    ...
    static async mapRequest(mapperToken, input, options = {}) { const result = await this.request(this.getMapperUrl(mapperToken, options), {method: 'POST',headers: {'Content-Type': 'application/json'},body: JSON.stringify(input ?? {})}); return this.result(result); }
    static async map(mapperToken, input, options = {}) { const result = await this.request(this.getMapperUrl(mapperToken, options), {method: 'POST',headers: {'Content-Type': 'application/json'},body: JSON.stringify(input ?? {})}); if (!result.ok) throw Object.assign(new Error(`Unable to evaluate mapper (${result.http_code})`), result); return result.data; }
    ...
    static async setValue(token, value) { return this.postRequest(`/v/${encodeURIComponent(token)}`, {value}); }
    static async getValue(token) { return this.getRequest(`/v/${encodeURIComponent(token)}`); }
